...
//! handle:
//!
//! 1. Spawns `arccode --worker-mode --task-file … --role … --worktree …
//!    --print --json` under a [`crate::child_process::Supervisor`] (cross-
//!    platform tree-kill).
//! 2. Parses stdout line-by-line. Each line is either:
//!    - an `AgentEvent` produced by `arccode-core` (tool start / result,
//!      usage, stop, error), or
...
pub struct WorkerSpec {
    /// Path to the arccode binary. Usually `std::env::current_exe()`.
    pub arccode_bin: PathBuf,
...
pub async fn run_worker(
    ...
    let mut sc = SupervisedCommand::new(&spec.arccode_bin);
    sc.command_mut()
        .arg("--worker-mode")
        .arg("--task-file")
        .arg(&task_path)
...
fn parse_line(line: &str) -> WorkerLine {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(line) {
        ...
        if v.get("type").is_some() {
            if let Ok(ev) = serde_json::from_value::<arccode_core::AgentEvent>(v) {
...
async fn forward_agent_event(
    ...
    task_id: &str,
    agent_id: &str,
    event: &arccode_core::AgentEvent,
) {
    match event {
        arccode_core::AgentEvent::ToolStart { name, .. } => {
            let _ = store
        ...
        arccode_core::AgentEvent::ToolResult { is_error, .. } if *is_error => {
...
fn write_task_file(task: &Task, worktree: &Path) -> Result<PathBuf, WorkerError> {
    // Put the task JSON inside the worktree's .arccode/ subdir so it's
    // visible to the worker without needing extra env vars.
    let dir = worktree.join(".arccode").join("pilot");
...
mod tests {
    ...
    async fn worker_pipeline_round_trip() {
        ...
        store
            .append(Event::TaskCreate {
                ...
                deps: vec![],
                writes: vec!["crates/arccode-cli/src/args.rs".into()],
                acceptance: vec![],
        ...
        let canned = concat!(
            ...
            "\n",
            r#"{"type":"tool_start","id":"call-1","name":"edit_file","input":{"path":"crates/arccode-cli/src/args.rs"}}"#,
            "\n",
            ...
            "\n",
            r#"{"event":"task_complete","summary":"Added --version-only flag to args.rs.","files_changed":["crates/arccode-cli/src/args.rs"]}"#,
            "\n",
    ...
    fn parse_line_handles_agent_event() {
        ...
        match parse_line(line) {
            WorkerLine::AgentEvent(arccode_core::AgentEvent::TextDelta { text }) => {
