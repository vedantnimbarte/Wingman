...
impl Provider for AnthropicProvider {
    ...
    async fn list_models(&self) -> Result<Vec<String>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| WingmanError::Provider(format!("anthropic list models request: {e}")))?;
        ...
        let json: Value = response
            .json()
            .await
            .map_err(|e| WingmanError::Provider(format!("anthropic list models parse: {e}")))?;
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<ProviderEventStream> {
        ...
        let stream = async_stream::try_stream! {
            while let Some(item) = events.next().await {
                let evt = match item {
                    Ok(e) => e,
                    Err(e) => Err(WingmanError::Provider(format!("sse error: {e}")))?,
                };
                ...
                let data: Value = match serde_json::from_str(&evt.data) {
                    Ok(v) => v,
                    Err(e) => {
                        tracing::warn!(target: "wingman::anthropic", "bad sse json: {e}: {}", evt.data);
                        continue;
                    }
                };
