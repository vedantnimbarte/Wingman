//! Interactive ratatui UI for `wingman pilot watch`.
//!
...
type LogSeverityLine = wingman_autonomous::dashboard::LogRow;
...
/// Ring the terminal bell (BEL) unless `WINGMAN_NO_BELL` is set. Writing the
/// control byte is safe under raw mode — it doesn't disturb the cursor.
fn emit_bell() {
    if std::env::var_os("WINGMAN_NO_BELL").is_some() {
        return;
    }
    use std::io::Write;
