//! `wingman diff` — interactive unified-diff hunk viewer with accept /
//! reject per hunk.
//!
//! Two modes:
//!   - `wingman diff <file>` → runs `git diff -- <file>` and walks each hunk
//!     against the working-tree file. Accepting keeps the change; rejecting
//!     restores the pre-change lines.
//!   - `wingman diff --patch <file.patch>` → applies a unified-diff file
...
pub async fn run(file: Option<String>, patch: Option<String>) -> Result<ExitCode> {
    let diff_text = if let Some(p) = patch {
        ...
        eprintln!("wingman: pass <file> or --patch <file>");
        return Ok(ExitCode::from(1));
    };

    if diff_text.trim().is_empty() {
        println!("wingman: no diff to review");
        return Ok(ExitCode::SUCCESS);
    }

    let files = parse_unified_diff(&diff_text);
    if files.is_empty() {
        eprintln!("wingman: no parseable file sections in diff");
        return Ok(ExitCode::from(1));
    }
    ...
    for (file_idx, fd) in files.into_iter().enumerate() {
        ...
        let path = match resolve_target(&fd) {
            Some(p) => p,
            None => {
                eprintln!("wingman: skipping file '{}' (no target path)", fd.new_path);
                continue;
            }
        };
        println!(
