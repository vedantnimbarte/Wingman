...
//! `wingman_ts::enclosing_symbol` for the symbol that contains each hunk.
...
pub fn annotate(project_root: &Path, fd: &FileDiff) -> FileAnnotation {
    #[cfg(feature = "treesitter")]
    {
        ...
        let Some(lang) = wingman_ts::Language::from_path(&target) else {
            return FileAnnotation {
                hunks: vec![None; fd.hunks.len()],
            };
        };
        ...
        let hunks = fd
            ...
            .map(|h| {
                ...
                wingman_ts::enclosing_symbol(lang, &content, probe)
