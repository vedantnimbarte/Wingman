...
//!
//! Wraps the per-server [`arccode_mcp::McpServer`] handles + their currently
...
use arccode_config::{global_config_path, Config, McpServerConfig};
use arccode_mcp::{McpServer, McpTool, McpToolHandle};
use arccode_tools::ToolRegistry;
...
impl McpRegistry {
    ...
    pub async fn seed(&self, mcp: &BTreeMap<String, McpServerConfig>) {
        ...
        for (name, cfg) in mcp {
            ...
            match arccode_mcp::connect(name, cfg).await {
                Ok(server) => {
    ...
    pub async fn add(&self, name: String, config: McpServerConfig) -> Result<(), String> {
        ...
        match arccode_mcp::connect(&name, &config).await {
            Ok(server) => {
                self.register_server_tools(&server, &mut entry.tool_names);
                entry.server = Some(server);
            }
    ...
    pub async fn connect(&self, name: &str) -> Result<(), String> {
        ...
        let server = arccode_mcp::connect(name, &config)
            .await
    ...
    fn register_server_tools(&self, server: &McpServer, into: &mut Vec<String>) {
        for tool in &server.tools {
            ...
            let adapter: Arc<dyn arccode_tools::Tool> = Arc::new(McpToolAdapter::new(handle));
