//! `arccode checkpoint` / `arccode undo` — git-stash based snapshot/restore.
//!
//! Each checkpoint is a `git stash push --include-untracked --keep-index` with
//! a recognizable label. `arccode undo` finds the most recent arccode stash
//! and `git stash pop`s it.

use anyhow::{Context, Result};
use std::process::{Command, ExitCode, Stdio};

const TAG: &str = "arccode-checkpoint";
...
pub async fn create(label: Option<String>) -> Result<ExitCode> {
    ...
    if !status.success() {
        eprintln!("arccode: `git stash push` failed (nothing to stash? or hooks?)");
...
pub async fn undo() -> Result<ExitCode> {
    ...
    if !out.status.success() {
        eprintln!("arccode: `git stash list` failed");
        return Ok(ExitCode::from(1));
    }
    ...
    let Some(stash_ref) = idx else {
        eprintln!("arccode: no checkpoint stash found (look for '{TAG}' in `git stash list`)");
        return Ok(ExitCode::from(1));
    };
    let status = Command::new("git")
    ...
    if !status.success() {
        eprintln!("arccode: `git stash pop {stash_ref}` failed (likely a conflict)");
        return Ok(ExitCode::from(1));
    }
