//! `arccode init` — scaffold an ARCCODE.md by introspecting the current project.
//!
//! Detects:
...
//!
//! Writes `<project-root>/ARCCODE.md`. Existing files are preserved unless
//! `--force` is passed.
...
pub async fn run(force: bool) -> Result<ExitCode> {
    ...
    let out_path = project.root.join("ARCCODE.md");

    if out_path.exists() && !force {
        eprintln!(
            "arccode: refusing to overwrite existing {} (re-run with --force)",
            out_path.display()
        );
...
fn render(s: &Summary) -> String {
    ...
    out.push_str(
        "This file is loaded into arccode's system prompt for every session in this \
