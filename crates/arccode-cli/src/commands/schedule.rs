//! `arccode schedule run` — fire any `[[schedule]]` entries whose cadence
//! has elapsed since their last successful run.
//!
//! Last-run timestamps live at `~/.arccode/schedule.json` as
//! `{ "<id>": <unix_secs> }`. Intended for cron-style invocation; we do
//! NOT spawn a daemon.

use anyhow::Result;
use arccode_config::{global_config_path, global_dir, Config, ProjectPaths, ScheduledTask};
...
pub async fn run(all: bool) -> Result<ExitCode> {
    ...
    if cfg.schedule.is_empty() {
        eprintln!("arccode: no [[schedule]] entries configured");
    ...
    if let Err(e) = save_state(&state) {
        eprintln!("arccode: warning, could not save schedule state: {e}");
    }
