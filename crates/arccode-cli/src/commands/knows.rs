//! `arccode knows` — render what Arc-Code knows about this project: stored
...
pub async fn run(cfg: Config) -> Result<ExitCode> {
    ...
    println!("What Arc-Code knows about {}", paths.root.display());
    println!();

    // Memories: global (~/.arccode/memory) + project (<root>/.arccode/memory).
    let global_mem = arccode_config::ensure_global_dir()
        .ok()
        .map(|d| d.join("memory"));
