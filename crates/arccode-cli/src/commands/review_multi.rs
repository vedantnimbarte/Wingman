//! `arccode review-multi` — fan a code-review prompt out across multiple
...
pub async fn run(pr: Option<String>, local_base: Option<String>, models_csv: String) -> Result<ExitCode> {
    let models: Vec<String> = models_csv
        ...
        .collect();
    if models.is_empty() {
        eprintln!("arccode: --models is empty");
        return Ok(ExitCode::from(1));
    }
    for m in &models {
        if !m.contains('/') {
            eprintln!("arccode: model entries must be provider/model — got '{m}'");
            return Ok(ExitCode::from(1));
        }
    }
    ...
    if transcripts.is_empty() {
        eprintln!("arccode: all reviewers failed");
        return Ok(ExitCode::from(1));
    }
