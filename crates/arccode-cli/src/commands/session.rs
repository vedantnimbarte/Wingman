//! `arccode session …` — list and fork session JSONL files.
...
async fn list(limit: usize) -> Result<ExitCode> {
    ...
    if !dir.exists() {
        eprintln!("arccode: no sessions yet in {}", dir.display());
...
async fn fork(src: String, at: Option<usize>) -> Result<ExitCode> {
    ...
    if !src_path.exists() {
        eprintln!("arccode: source session not found: {}", src_path.display());
        return Ok(ExitCode::from(1));
    }
    let dest = arccode_session::fork_session(&src_path, &project.sessions_dir, at)
        .await
