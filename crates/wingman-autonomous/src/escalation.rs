...
/// True when `branch` is inside the pilot's own `wingman/auto/*` namespace,
...
pub fn is_pilot_namespace(branch: &str) -> bool {
    ...
    b.starts_with("wingman/auto/")
}

/// J15 — a force-push is allowed only inside the `wingman/auto/*` namespace.
...
mod tests {
    ...
    fn empty_state() -> RunState {
        RunState::new("r1", "g", "abc", "wingman/auto/r1")
    }
    ...
    fn force_push_allowed_inside_pilot_namespace() {
        assert!(force_push_trigger("wingman/auto/2026-06-02-x", true).is_none());
        assert!(force_push_trigger("refs/heads/wingman/auto/r1", true).is_none());
