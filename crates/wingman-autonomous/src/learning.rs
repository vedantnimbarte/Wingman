...
//!
//! Three mechanisms, all reading/writing under `~/.wingman/`:
//!
//! 1. **Per-`(role, model)` stats** in `~/.wingman/stats.jsonl` — one
...
//!    routing.
//! 2. **Per-role lessons** in `~/.wingman/agents/<role>.lessons.md` —
...
/// One append-only line in `~/.wingman/stats.jsonl`. Written once when a
...
/// Conventional stats path: `<home>/.wingman/stats.jsonl`.
pub fn stats_path(home: &Path) -> PathBuf {
    home.join(".wingman").join("stats.jsonl")
}
...
/// Conventional lessons path: `<home>/.wingman/agents/<role>.lessons.md`.
...
mod tests {
    ...
    fn stats_roundtrip_through_file() {
        let dir = std::env::temp_dir().join(format!("wingman-learn-{}", std::process::id()));
    ...
    fn load_stats_missing_file_is_empty() {
        let path = std::env::temp_dir().join("definitely-not-here-wingman.jsonl");
        let _ = fs::remove_file(&path);
    ...
    fn load_stats_skips_corrupt_lines() {
        let dir =
            std::env::temp_dir().join(format!("wingman-learn-corrupt-{}", std::process::id()));
    ...
    fn append_lesson_seeds_heading_then_appends() {
        let dir = std::env::temp_dir().join(format!("wingman-lessons-{}", std::process::id()));
    ...
    fn load_lessons_roundtrips_what_append_wrote() {
        let dir = std::env::temp_dir().join(format!("wingman-lessons-load-{}", std::process::id()));
