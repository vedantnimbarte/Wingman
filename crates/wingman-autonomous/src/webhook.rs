...
//!
//! **STATUS: partially live.** `hmac_sha256`, `to_hex`, and
//! `header_boundary_and_len` now back the authenticated Slack intake listener
//! (`wingman pilot intake slack`). The HTTP receiver itself (`serve`,
//! `serve_listener`, `handle_connection`) still has no production caller, and
//! `[pilot.daemon].webhook_secret` is parsed but unread.
//!
//! Known gap if it is ever wired: there is no timestamp/replay window here —
//! a captured signed request replays forever. The Slack path adds one; this
//! one would need the same before being exposed.
...
/// `X-Wingman-Signature: sha256=<hex>` header computed over the raw request
...
fn extract_signature(headers: &str) -> Option<String> {
    for line in headers.lines() {
        if let Some((k, v)) = line.split_once(':') {
            if k.trim().eq_ignore_ascii_case("x-wingman-signature") {
                let v = v.trim();
...
/// tests) can produce the `X-Wingman-Signature` header value.
...
mod tests {
    ...
    fn authenticated_request_trusts_allowlisted_author() {
        ...
        let client = std::thread::spawn(move || {
            ...
            let req = format!(
                "POST /goals HTTP/1.1\r\nHost: localhost\r\nX-Wingman-Signature: sha256={sig}\r\nContent-Length: {}\r\n\r\n{}",
                body.len(),
                body
            );
            s.write_all(req.as_bytes()).unwrap();
    ...
    fn unsigned_request_is_rejected_when_secret_required() {
        ...
        let client = std::thread::spawn(move || {
            ...
            let req = format!(
                "POST /goals HTTP/1.1\r\nX-Wingman-Signature: sha256=deadbeef\r\nContent-Length: {}\r\n\r\n{}",
                body.len(),
                body
            );
            s.write_all(req.as_bytes()).unwrap();
