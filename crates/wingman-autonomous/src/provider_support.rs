...
//! Not every backend in `wingman-providers` reliably does that — some
//! local-model setups (LM Studio, vLLM, Ollama) will load any model the
//! user picks, including ones that have no tool-use training.
//!
//! This module gates `wingman pilot run` on a static support table:
//!
//! | Tier         | Behaviour                                                 |
//! | ------------ | --------------------------------------------------------- |
...
/// `wingman pilot run`'s startup banner.
    ...
    if classify(provider_id) == ProviderSupport::Unsupported {
        Err(format!(
            "provider {provider_id} is marked unsupported for pilot mode (no structured tool use). \
             Use --model <other-provider>/<model> or `wingman config set default_provider`."
        ))
    } else {
        Ok(())
    }
