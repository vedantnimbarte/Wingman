...
pub fn extract_keywords(goal: &str) -> Vec<String> {
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        // Stay verb/noun-y. Drop quintessential stopwords + wingman
...
/// Skips `.git`, `target`, `node_modules`, `.wingman/worktrees`, and
    ...
    while let Some(dir) = stack.pop() {
        ...
        for e in entries.flatten() {
            ...
            if e.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                ...
                // Skip nested .wingman/worktrees so we don't recurse into
                // every per-task worker tree.
                if name_s == ".wingman" {
                    // descend into .wingman but skip its worktrees dir
                    stack.push(path);
                    continue;
                }
                if name_s == "worktrees" {
