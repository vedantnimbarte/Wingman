...
Goal: make Wingman measurably **faster**, produce **more trustworthy results**, and give users
...
> **Wingman is the coding agent that already knows your repo, picks the right model for every
...
Wingman already has partial infrastructure for all of them.

---

## Competitive gaps we attack

| Weakness in competitors | Wingman asset that exploits it |
|---|---|
| Single-provider lock-in (Claude Code → Anthropic, Codex → OpenAI) | `Provider` trait + 9 adapters + 70-odd OpenAI-compatible endpoints (`wingman-providers`) |
| No memory between sessions; repo knowledge rebuilt every run | Learning loop: memories, skill stats, session embeddings (`wingman-learn`) |
| Search = grep fan-out; slow and token-hungry on large repos | RAG index + tree-sitter semantic chunks (`wingman-rag`, `wingman-ts`) |
| "Done" without proof; confidently broken output | Turn gate, checkpoint verify, acceptance checks (`wingman-autonomous`) |
...
1. **Task-class router** (`wingman-core`): classify each model call as one of
...
2. **Learned routing** (`wingman-learn`): the skill-outcome scoring in `learn.db` already
   records success/failure per task. Extend the schema with `(task_class, model, outcome)` and
   surface `wingman router stats` showing which model wins per class *in this repo*. Opt-in
   auto-routing once enough samples exist.
3. **Local-model preset:** a curated `[router]` profile that keeps `search_summarize`,
   `commit_message`, and `compaction` on a local Ollama model (`wingman discover` already finds
   them). Privacy story: "simple steps never leave your machine."

**Integration points:** model resolution in `wingman-core` (config defaults → CLI flags →
...
**Status today:** `wingman-rag` builds a per-project SQLite index (`.wingman/index.db`) with
embedded semantic chunks from `wingman-ts`; a `notify`-based file watcher (500 ms debounce)
exists; `semantic_search` is a registered tool. What's missing is *always-on freshness* and
*default-on usage*.

**Build:**

1. **`wingman indexd`** — a detached background process per project that keeps `index.db`
...
**Integration points:** `wingman-rag` (index, watcher), `wingman-ts` (symbols),
`crates/wingman-tools/src/registry.rs` (new tools), TUI startup path.

---

## Pillar 3 — Verification receipts ("proves its work")

**Status today:** `wingman-autonomous` has `checkpoint::verify()`, executable acceptance checks
...
3. **Verification receipt in the TUI/headless output:** a compact block on completion —
   ...
   Receipts are also written to the session JSONL so `wingman session list` shows which past
   runs were verified.
4. **Config:** `[verify] turn_gate = "auto" | "<cmd>" | "off"`, `affected_tests = true`.

**Integration points:** stop/turn-end logic in `wingman-core`,
`LearningHook::after_turn()/after_stop()`, reuse of acceptance-check executors from
`wingman-autonomous` (consider lifting them into `wingman-core` or a shared crate).
...
**Status today:** 4 memory types as markdown+frontmatter in `~/.wingman/memory/` (global) and
`<project>/.wingman/memory/` (project); skill stats in `learn.db`; session embeddings in
...
2. **Team-shared memory:** document and support committing `.wingman/memory/` (and optionally a
   sanitized skills subset) to git. Add `wingman memory sync` to merge teammate memories without
   clobbering local ones. A new teammate's agent starts with the team's accumulated knowledge —
   the switching-cost moat.
3. **`wingman knows`:** a command + TUI panel rendering "what I know about this project"
...
**Integration points:** `wingman-learn` (memory store, hooks), `wingman-skills`,
`wingman-rag` (staleness checks), TUI panel, CLI subcommand in `wingman-cli`.

---

## Supporting bets (cheaper, scheduled opportunistically)

- **Speculative parallelism** (`wingman-core`): while the model streams, pre-read files it is
...
- **Repo health autopilot:** once Pilot M2/M3 land, a low-priority scheduled run (existing
  ...
  PRs. Converts Wingman from "tool I invoke" to "teammate that's always producing value."
...
| **D2** | `wingman indexd` + search escalation + symbol-graph tool | 🔶 **Partial on `main`, rest in open PRs.** On `main`: `wingman indexd` as a *foreground* watcher with a pidfile (`--status`), and `who_calls` as a tree-sitter whole-word name match annotated with the enclosing symbol. **In [#250](https://github.com/vedantnimbarte/Wingman/pull/250) (open):** `indexd start/stop/status` (detached, real process-liveness check, TUI skips its own indexer when a daemon is live), import-ranked `read_file` prefetch, and search escalation (prompt + `grep` description point at `semantic_search` first; a per-turn hint block of indexed locations under `[learn].search_hint_tokens`). **In [#249](https://github.com/vedantnimbarte/Wingman/pull/249) (open):** `who_calls` asks a language server first (`callHierarchy/incomingCalls`, then `references`, then the name match). **In [#248](https://github.com/vedantnimbarte/Wingman/pull/248) (open):** C++/Java/Kotlin grammars and incremental reparse on re-chunk. Unvalidated live: no real language server was available (tested against an in-process fake), the Unix liveness/`setsid` code has not been compiled locally, and no session has been run against a live daemon. |
| **D3** | Default-loop turn gate + affected tests + verification receipts | ✅ **Core shipped** (2026-06-12): `TurnGate` trait + gate retry loop in `AgentLoop`, `AgentEvent::Verification` receipts in TUI/headless, `[verify]` config (`turn_gate = "auto"/"off"/cmd`), auto-detection per ecosystem (`ShellTurnGate` in `wingman-cli/src/runtime.rs`). Affected tests on `main` are **crate-level** (`AffectedTestsGate` runs the changed crates' tests). **In [#249](https://github.com/vedantnimbarte/Wingman/pull/249) (open):** symbol-level narrowing — edited symbols → referencing tests via LSP `references` or the tree-sitter index, falling back to crate-level whenever a change can't be tied to a symbol. Known gap: tests that reach an edit only through non-test code aren't mapped; a cold server's *partial* references answer would pick too few tests (not detectable without a real server). |
| **D4** | Post-session distillation + `wingman knows` + team memory sync | 🔶 **Mostly shipped on `main`**: `wingman knows`; `wingman memory export/import/diff` for sharing; `wingman distill` stages facts for `wingman memory review` rather than writing them silently; memory staleness flags memories naming files that no longer exist. **In [#249](https://github.com/vedantnimbarte/Wingman/pull/249) (open):** staleness also resolves named symbols (tree-sitter, then `workspace/symbol`). **In [#250](https://github.com/vedantnimbarte/Wingman/pull/250) (open):** `distill`/`explain` resolve through `[router.classes].summarize` when that class is routed, keeping `fast_model` otherwise. |
| **D5** | Learned routing, speculative parallelism, rewind timeline | 🔶 **Partial on `main`, rest in open PRs.** On `main`: `wingman router stats` over routing rows that only `--print` records; sibling-file prefetch on `read_file`; `wingman rewind` over per-edit checkpoints. **In [#250](https://github.com/vedantnimbarte/Wingman/pull/250) (open):** routing rows from TUI and pilot workers too, durable PR verdicts (`wingman router backfill`: revert / red CI / reopened issue / blame survival), and opt-in learned routing (`[router].learned_min_samples`, off by default). **In [#251](https://github.com/vedantnimbarte/Wingman/pull/251) (open):** a turn-grouped rewind timeline with diff preview and restore-to-point in the TUI (`/rewind`), HTTP and panel. Unvalidated live: the `gh` post-merge checks have never run against a real merged PR, learned routing has never picked a model in a real session, and the rewind UI has not been driven by hand. |

The metrics below are now recorded: **in [#251](https://github.com/vedantnimbarte/Wingman/pull/251) (open)**,
turn stop records carry time-to-first-output and the last receipt, and `wingman metrics`
(also `wingman knows`, `GET /v1/projects/{p}/metrics`, the panel's Insights page, and
`wingman bench --markdown`) reports them. No figure has been collected from a live model yet.
...
*Sequencing table refreshed 2026-09-15 against `main` (0.4.0) and the open PRs
#247–#253; "in #N (open)" means the code is on that branch, not merged.*
