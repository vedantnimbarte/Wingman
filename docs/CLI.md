...
| `mcp-serve`          | Expose Wingman itself as an MCP server over stdio (tools + memory resources, plus `pilot_status` and, under `--mode auto-edit`, `pilot_run`). Read-only by default; raise with `--mode`. |
