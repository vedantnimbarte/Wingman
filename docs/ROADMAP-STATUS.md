...
| Daemon `auto_dispatch` | **Issue → PR and the `pr_checks` CI-fix round ran live on 2026-09-17**, against a private scratch repo on `claude-code/haiku`: a trusted, labelled issue auto-dispatched a run that opened a PR, its CI failed, one `pr_checks` round read the failed log and pushed the fix, and CI went green. The next cycles did not dispatch again. Still not run live: a `pr_reviews` round (review threads, reply and resolve), and an auto-merge-armed PR | [#34](https://github.com/vedantnimbarte/Wingman/issues/34) |
